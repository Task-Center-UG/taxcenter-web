...
export default function BincangSoreDetailPage() {
  const params = useParams();
  const id = params.id as string;

  const { data, isLoading, isError } = useGetData<AfternoonTalkDetail>({
    key: ["afternoon-talk-detail", id],
    url: `/afternoon-talk/${id}`,
  });
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8F9FA] gap-3">
        ...
        <p className="text-gray-500 font-medium">Memuat detail bincang sore...</p>
      </div>
    );
  }
  ...
  if (isError || !item) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8F9FA] px-4 text-center">
        ...
        <Link
          href="/edukasi-pajak/bincang-sore"
        ...
        <div className="flex justify-end">
          <Link href={item.video_url} target="_blank">
            <button className="inline-flex items-center gap-2 bg-[#F97316] hover:bg-orange-600 text-white font-medium py-3 px-6 rounded-full transition-colors">
              Lihat Video <PlayCircle className="w-4 h-4" />
            </button>
          </Link>
        </div>
      </div>
