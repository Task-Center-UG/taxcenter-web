...
export default function VideoPembelajaranDetailPage() {
  const params = useParams();
  const id = params.id as string;

  const { data, isLoading, isError } = useGetData<TaxLearningVideoDetail>({
    key: ["tax-learning-video-detail", id],
    url: `/tax-learning-video/${id}`,
  });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8F9FA] gap-3">
        ...
        <p className="text-gray-500 font-medium">
          Memuat detail video pembelajaran...
        </p>
      </div>
    );
  }
