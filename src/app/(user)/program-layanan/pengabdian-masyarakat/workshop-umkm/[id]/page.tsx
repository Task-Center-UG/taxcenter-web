...
export default function WorkshopUmkmDetailPage() {
  const params = useParams();
  const id = params.id as string;

  const { data, isLoading, isError } = useGetData<TrainingDetail>({
    key: ["umkm-workshop-detail", id],
    url: `/training/${id}`,
  });
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8F9FA] gap-3">
        ...
        <p className="text-gray-500 font-medium">Memuat detail workshop UMKM...</p>
      </div>
    );
  }
